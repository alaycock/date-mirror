...
export default class DateMirror extends Plugin {
  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.vault.on("rename", (file, newPath) => {
        if (file instanceof TFile) {
  ...
  async updateFilename(file: TFile) {
    ...
    if (cache?.frontmatter?.[this.settings.dateProperty]) {
      ...
      const momentDate = window.moment(date, true);
      if (!momentDate.isValid()) {
        return;
      }

      const formattedDate = momentDate.format(this.settings.dateFormat);
      ...
      if (newName !== currentName) {
        ...
        console.debug("renaming file", file.path, "->", newPath);
  ...
  private replaceDateInFilename(filename: string, newDate: string): string {
    ...
    const datePattern = this.createDatePatternFromFormat(
  ...
  // TODO: Complete this list with all valid date formatting options.
      ...
      await this.app.fileManager.processFrontMatter(file, (frontmatter) => {
        ...
        if (frontmatter[this.settings.dateProperty] !== newDate) {
          console.debug(
            ...
            frontmatter[this.settings.dateProperty],
            "->",
            newDate
          );
...
class SampleSettingTab extends PluginSettingTab {
  ...
  private getAllFrontmatterProperties(): string[] {
    ...
    const cachedProperties: Record<string, CachedProperty> = (this.app as any)
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "The date property that should be kept in-sync with the filename"
      )
      .addDropdown((dropdown) => {
        ...
        const enabled = frontmatterProperties.length !== 0;
        
        if (!enabled){
          dropdown.setDisabled(true);
          dropdown.addOption('DEFAULT', 'No date properties exist')
        }
    ...
    // Date format setting
    // I found this setting formatting in the Obligator plugin
    let dateFormatter: MomentFormatComponent;
    ...
    const dateFormatEl = settingDateFormat.descEl.createEl("b", {
      cls: "u-pop",
      text: "test",
    });
    // @ts-ignore
    dateFormatter.setSampleEl(dateFormatEl);
